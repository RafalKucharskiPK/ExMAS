# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 15:04:03 2020
...
def nodes_gdf(df, _params):
    geometry = [Point(xy) for xy in zip(df.x, df.y)]
    df = df.drop(['x', 'y'], axis=1)
    return gp.GeoDataFrame(df, crs=_params.crs, geometry=geometry)

def read_postcodes(_params):
    PC4 = gp.read_file(_params.paths.postcodes)
    PC4 = PC4.to_crs('epsg:4326')
...
def albatross_import(_inData,_params):
    # read the albratross .txt file and merge them into pd.DataFrame
    ...
    _inData.albatross.PC4 = read_postcodes(_params)[['PC4','geometry']]
    _inData.albatross.gdf_nodes = nodes_gdf(_inData.nodes,_params)
...
def generate_demand_albatross(_inData,_params, copy = True, sample = None):
    ...
    #    #    request.dist = _inData.skim[request.orig in][request.destination]
    #     
    #    #    t.loc[i] = request 
